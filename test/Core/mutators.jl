...
@testset verbose = true "Mutators $(repeat("-", 45))" begin
    ...
    @testset verbose = true "$(@sprintf "%-54s" "Crankshaft")" begin
        ...
        T = eltype(pose.state)
        @test CB_pos_2 == MVector{3, T}([3.8810175757721384, -4.721163091936314, -1.3002006003050992])
    end
    ...
    @testset verbose = true "$(@sprintf "%-54s" "Rotation Rigid Body")" begin
        ...
        T = eltype(pose.state)
        @test pos_2_1 == MVector{3, T}([1.7573120826160027, -1.4568106376562309, 0.7092469773463803])
        @test pos_2_2 == MVector{3, T}([4.237565726518101, -4.178262545553745, 0.18364182332261403])
        @test pos_2_3 == MVector{3, T}([7.863391631896685, -4.874122906544416, -0.2690548282792112])
    end
