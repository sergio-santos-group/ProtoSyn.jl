@generated function serial_soa(state::State{T}, cutoff::Float64, ::Type{Val{DOF}}) where {DOF, T <: AbstractFloat}
    # println("generating serial_soa with DOF = $DOF")
    fbody = quote
        coords = state.coords
        forces = state.forces
        natoms = state.size
...
@generated function serial_soa(state::State{T}, vlist::VerletList, ::Type{Val{DOF}}) where {DOF, T <: AbstractFloat}
