...
const n_i_search_steps         = 200000 # Number of steps on each Monte Carlo Search inner cycle;
const n_o_search_steps         = 10     # Number of ILSRR outer cycles to perform;
const n_init_min_steps         = 10000  # Number of steps for Steepest Descent minimization (initial long minimization);
const n_refine_steps           = 250    # Number of steps for Monte Carlo Refinement;
const n_refine_min_steps       = 20000  # Number of steps for Steepest Descent minimization (every step of Monte Carlo Refinement);
...
const print_sts_every_ic       = 10000  # Print system status every n steps (Inner cycle);
...
const print_str_every_ic       = 25000  # Print structure to files every n steps;
...
const max_step                 = 1e-4   # Maximum step allowed in Steepest Descent;
const f_tol                    = 50.0   # Minimum force tolerated. The minimization is considered converged when forces are below the defined value;
...
const refine_temp_static       = 200.0  # Temperature for Metropolois acceptance in refinement (0.0 performs a Random Search where only better structures are accepted);
...
const contact_min_distance     = 0.2    # Minimum distance (in nm) between the atoms involved in the contact;
