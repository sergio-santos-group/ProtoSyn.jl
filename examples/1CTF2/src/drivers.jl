...
soft_dh_mutator = Mutators.Dihedral.DihedralMutator(nb_dhs, () -> (randn() * soft_dh_mutator.step_size), soft_dh_p_mut, soft_dh_step_size)
soft_cs_mutator = Mutators.Crankshaft.CrankshaftMutator(nb_phi_dhs, () -> (randn() * soft_cs_mutator.step_size), soft_cs_p_mut, soft_cs_step_size)
...
function dh_cs_hard_sampler!(st::Common.State)
    ...
    println("(ILSRR) Perturbator ▶️ Dihedral: $dm | Crankshaft: $cm")
    return Dict("d" => dm, "c" => cm)
end
...
i_search_driver = Drivers.MonteCarlo.Driver(dh_cs_reg_sampler!, amber_cr_evaluator!, i_search_temp_init, n_i_search_steps,
    print_status_mc, quench_temperature, print_structure, adjust_step_size)
