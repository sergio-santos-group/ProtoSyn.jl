...
# Output frequency:
const print_sts_every_ic       = 500    # Print system status every n steps (Inner cycle);
...
const print_str_every_oc       = 0      # Print current structure to files every n steps;
const print_str_every_rfnm     = 0      # Print current structure to files every n steps in Monte Carlo Refinement;

const print_ene_every_ic       = 1000   # Print energy information every n steps;

# Slope anaylis parameters
const evaluate_slope_every     = 500    # Calculate the energy decrease slope of the last n accepted steps;
const evaluate_slope_threshold = -0.5   # Convergence criteria: if the energy decrease slope is smaller than this threshold, the system moves on;
const continue_after_n_attemps = 10     # Exit the ISLRR algorithm after n jumps failed to produce a new homebase; 
...
const contact_force_constant   = 2e3    # Force constant for contact distance FBRs;
const contact_min_distance     = 0.8    # Minimum distance (in nm) between the atoms involved in the contact;

# Dihedral FBR settings
const dihedral_force_constant  = 2e3    # Force constant for dihedral FBRs; 
...
const ss                       = "CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC"

# Input files:
const input_pdb                = "data/PRT_no_sc.pdb"
const input_contact_map        = "data/contact_map_raptorx_PRT.txt"
...
const out_destination          = open("out/run.log", "w")
const outer_best_destination   = open("out/best.pdb", "w")
const status_destination       = [log_destination, out_destination]
const out_ene                  = open("out/energy.out", "w")
const energy_components_list   = ["eTotal", "eBond", "eAngle", "eDihedral", "eCoulomb", "eCoulomb14", "eLJ", "eLJ14", "eDistanceFBR", "eSol", "eH", "eDihedralFBR"]
